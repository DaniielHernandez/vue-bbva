      ...
      <v-card>
        ...
        <v-card-text>
          <v-container class="pt-0">
            <v-row>
              <v-col cols="12">
                <bbva-web-form-text @input="(ev) => setValue('fullName', ev)" :class="styles.textInput"
                  :value="inputs.fullName.value" :invalid="Boolean(inputs.fullName.error)"
                  :error-message="inputs.fullName.error" label="Nombre y apellidos"></bbva-web-form-text>
              </v-col>
              <v-col cols="12">
                <bbva-web-form-text @input="(ev) => setValue('email', ev)" :value="inputs.email.value"
                  :class="styles.textInput" :invalid="Boolean(inputs.email.error)" :error-message="inputs.email.error"
                  label="Email"></bbva-web-form-text>
              </v-col>
              <v-col cols="12">
                <bbva-web-form-text @input="(ev) => setValue('employeeId', ev)" :value="inputs.employeeId.value"
                  :class="styles.textInput" :invalid="Boolean(inputs.employeeId.error)"
                  :error-message="inputs.employeeId.error" label="ID usuario"></bbva-web-form-text>
              </v-col>
              <v-col cols="12">
                <bbva-web-form-select @change="(ev) => setValue('geographyN1Id', ev)"
                  :value="inputs.geographyN1Id.value" :invalid="!!inputs.geographyN1Id.error"
                  :error-message="inputs.geographyN1Id.error" label="Geografía">
                  <bbva-web-form-option v-for="option in catalogs.geographyN1Id" :key="option.id"
              ...
              <v-col cols="12">
                <bbva-web-form-select v-if="addBu" @change="(ev) => setValue('buId', ev)" :value="inputs.buId.value"
                  :invalid="!!inputs.buId.error" :error-message="inputs.buId.error" label="Business unit">
                  <bbva-web-form-option value="all"><span class="pl-4">Todas</span></bbva-web-form-option>
                  <bbva-web-form-option v-for="option in catalogs.buId" :key="option.id" :value="option.id"><span
                      class="pl-4">{{ option.label }}</span></bbva-web-form-option>
                </bbva-web-form-select>
              </v-col>
              <v-col cols="12">
                <bbva-web-form-select @change="(ev) => setValue('roleId', ev)" :value="inputs.roleId.value"
                  :invalid="!!inputs.roleId.error" :error-message="inputs.roleId.error" label="Rol">
                  <bbva-web-form-option v-for="option in catalogs.roleId" :key="option.id" :value="option.id"><span
                      class="pl-4">{{ option.label }}</span></bbva-web-form-option>
                </bbva-web-form-select>
              </v-col>
              <v-col cols="12">
                <bbva-web-form-select @change="(ev) => setValue('profileId', ev)" :value="inputs.profileId.value"
                  :invalid="!!inputs.profileId.error" :error-message="inputs.profileId.error" label="Perfil">
                  <bbva-web-form-option v-for="option in catalogs.profileId" :key="option.id" :value="option.id"><span
                      class="pl-4">{{ option.label }}</span></bbva-web-form-option>
                </bbva-web-form-select>
              </v-col>
              <v-col cols="12">
                <bbva-web-form-select @change="(ev) => setValue('areaId', ev)" :value="inputs.areaId.value"
                  :invalid="!!inputs.areaId.error" :error-message="inputs.areaId.error" label="Cargo/Área">
                  <bbva-web-form-option v-for="option in catalogs.areaId" :key="option.id" :value="option.id"><span
                      class="pl-4">{{ option.label }}</span></bbva-web-form-option>
                </bbva-web-form-select>
              </v-col>
            </v-row>
        ...
        <v-card-actions class="pl-10 pb-10">
          <bbva-button-default :disabled="!valid" text="Continuar" @click="onSubmit"></bbva-button-default>
          <div class="pl-8">
            <bbva-button-default text="Cancelar" variant="link" class="h-75" @click="close"></bbva-button-default>
          </div>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    loadCatalogs() {
      const mockCatalogs = [
        ...
        },
        {
          key: 'buId',
          data: [
            { id: 1, name: 'Unidad de Negocio A' },
            { id: 2, name: 'Unidad de Negocio B' }
          ]
        },
        {
          key: 'geographyN1Id',
          data: [
            { id: 1, name: 'Norteamérica' },
            { id: 2, name: 'Latinoamérica' },
            { id: 3, name: 'Europa' }
          ]
        },
        {
      ...
      mockCatalogs.forEach(({ key, data }) => {
        // Simula la transformación de la data como lo haría parseCatalog
        this.catalogs[key] = parseCatalog(data);
      });
    },
    /*
     * It sets the values
