        ...
        <v-table class="data-table" fixed-header>
          <thead>
            <th
              ...
              :class="`${getStickyHeaderClass(index)} text-left header-response-time`"
              :style="'padding-left: 15px;' + (index === 0 ? 'min-width: 200px;' : 'min-width: 70px;')"
              :width="index === 0 ? '400px;' : ''"
              :key="`response-time-heaer${index}`">
              <div class="d-flex">
              <bbva-button-default
                v-if="index === 0"
                variant="link"
              ...
              <span
                class="text-header-response-time">
                {{header.label}}
                </span>
              </div>
            </th>
            <th
              v-if="expandMonth"
              class="text-left header-response-time bg-white text-header-response-time"
          ...
          <tbody>
            <tr
              ...
              :key="`row-response-time-${indexRow}`"
            >
              <td
                :class="`
                    border-gray
                  ...
                  `"
                >
                <div v-if="!!row.cfs" class="fc-white fw-500 fs-14" >
                  <a
              ...
              <td :class="`
               ...
               <td :class="`
                ...
                <span
                  style="padding-bottom: 27px;"
                  >
                  {{ [null, undefined ].includes(row.threshold) ?  ''  : formatNumber(row.threshold) }}
                </span>
              </td>
              <td
                v-for="(col, indexCol) in months"
                :key="`col.response-time-${indexCol}`"
...
/* constants */
import {
  filtersResponseTimes, headersCFS, nestedFilterFields, validationsFilterFields
} from '@/constants/responseTimesCfs';
/* requests */
import {
  getResponseTimesCFSAPI,
  getResponseTimesCBPAPI,
  getResponseTimesOperativeAPI,
  getExcelFileResponseTimeCfs
} from '@/requests/responseTimes';
/* utils */
import {
...
export default {
  ...
  data: () => ({
    ...
    monthsKeys: [
      'annual', 'january', 'february', 'march', 'april', 'may', 'june', 'july', 'august', 'september',
    ...
    requestData: getResponseTimesCFSAPI,
  ...
  methods: {
    ...
    watchItem(cbpId, operativeId) {
      ...
      this.filters.values.forEach((item) => {
        if( ['cfsId', 'buId'].includes(item.name) ) {
          return;
        }
        query +=  item.name + '=' + item.value + '&';
      })
      const cfsId = this.filters.values.find( (item) => 'cfsId' === item.name );
      const buId = this.filters.values.find( (item) => 'buId' === item.name );
      query += `&buId=${buId.value}`
      ...
      this.$router.push(
        `/response-times/cfs-cbp-operative/detail?${query}`
      );
    },
    /*
    ...
    setTitle(bu, cfs) {
      ...
      const title = cfs && bu ? `Tiempos de respuesta - ${bu} - ${cfs}` : 'Tiempos de respuesta';
      const subtitle = `Resultados al ${ formatStringDate( date ) }`;
      this.emitter.emit("responseTimesCfsTitle", { title, subtitle });
    },
    /*
    ...
    updateHeaders() {
      ...
      this.months = [...this.monthsKeys].splice(0, this.getLastMonth() + 1);
    },
    /*
    ...
    setFilters() {
      ...
      this.radioFilters.compliance = this.$route.query.compliance === 'true';
    ...
    searchData(filters = []) {
      ...
      const bu = filters.find( (item) => item.name === 'buId' );
      const filtersRequest = [ 
        ...filters.filter( (item) => !['cfsId', 'buId'].includes(item.name) ),
        { name: 'decimals', value: this.radioFilters.decimals },
        { name: 'compliance', value: this.radioFilters.compliance },
        { name: 'cfsId', value: cfs.value },
        { name: 'buId', value: bu.value }
      ];

      this.emitter.emit("load", { loading: true });
      Promise.all([
        getResponseTimesCFSAPI( filtersRequest ),
        getResponseTimesCBPAPI( filtersRequest ),
        getResponseTimesOperativeAPI( filtersRequest )
      ]).then( (values) => {
    ...
    loadCatalogs() {
      ...
      Promise.all([
        ...requests.map( (request) => request(1,3000) ),
      ]).then(async (values) => {
